	...
	switch (def_zone)
		...
		if(BP_L_HAND, BP_R_HAND)
			...
			if(c_hand && (stun_amount || agony_amount > 10))
				msg_admin_attack("[src.name] ([src.ckey]) was disarmed by a stun effect", src)
		...
		if(ismob(O.thrower))
			...
			if(assailant)
				...
				if(!istype(src,/mob/living/simple_animal/mouse))
					msg_admin_attack("[src.name] ([src.ckey]) was hit by a [O], thrown by [M.name] ([assailant.ckey])", src)
